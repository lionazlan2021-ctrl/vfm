...
export default function SellerCard({
  ...
  return (
    <div
      ...
      style={{
        background: "#111815",
        border: `1px solid ${rank === 1 ? rc.color + "38" : "rgba(45,190,95,0.09)"}`,
    ...
    >
      {rank === 1 && (
        <div
          ...
          style={{ background: `linear-gradient(90deg,${rc.color},#1d9648)`, color: "#000" }}
        >
          ✦ BEST PRICE
        </div>
      )}
      ...
      <div className="flex gap-3 items-start" style={{ marginTop: rank === 1 ? 8 : 0 }}>
        <div
        ...
        >
          {!imgErr && listing.imageUrl ? (
            ...
            <img src={listing.imageUrl} alt="" onError={() => setImgErr(true)} className="w-full h-full object-cover rounded-[11px]" />
          ) : (
        ...
        <div className="flex-1 min-w-0" style={{ paddingRight: 56 }}>
          <div className="flex items-center gap-1.5 mb-1 flex-wrap">
            <span className="text-[15px]">{rc.icon}</span>
          ...
          <div className="text-[21px] font-extrabold leading-tight" style={{ color: rc.color, fontFamily: "JetBrains Mono, monospace" }}>
            {listing.price}
          </div>
          {listing.originalPrice && (
            <div className="text-[11px] line-through" style={{ color: "#3d5542" }}>
              {listing.originalPrice}
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-2 gap-1.5 my-3">
        {[
          ...
          { icon: "⭐", label: "Seller", val: listing.sellerRating ? `${listing.sellerRating}/5` : "Verified" },
        ].map((m) => (
      ...
      <div className="flex gap-1.5 mt-2.5">
        ...
        <button onClick={() => setShowPay((p) => !p)} className="btn-ghost px-[11px] py-2 text-[13px]" title="Payment plans">
          💳
        </button>
      </div>
      ...
      {open && (
        <div className="mt-2.5" style={{ animation: "fadeUp .3s ease" }}>
          {(listing.pros?.length || listing.cons?.length) ? (
            <div className="grid grid-cols-2 gap-2 mb-2.5">
              <div className="rounded-[10px] p-2.5" style={{ background: "rgba(45,190,95,0.07)", border: "1px solid rgba(45,190,95,0.15)" }}>
              ...
              <div className="rounded-[10px] p-2.5" style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.14)" }}>
          ...
          {listing.aiReason && (
            <div
              ...
              style={{ background: "rgba(45,190,95,0.05)", border: "1px solid rgba(45,190,95,0.18)", borderLeft: "2px solid #2dbe5f", color: "#8aaa8e" }}
            >
